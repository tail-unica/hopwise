...
**hopwise is based on RecBole, so the data formatting is similar.**

hopwise not only implements lots of popular recommender models, but also collects and releases datasets.
...
required by different recommendation tasks, hopwise designs an input data format called :doc:`datasets/atomic_files`. All the input data should be
...
Plus, hopwise supports both explicit feedback (labeled data) scenes and implicit feedback (unlabeled data) scenes. For explicit feedback scenes,
users can set the `LABEL_FIELD` in the config and hopwise will train and test model based on the label. For implicit feedback scenes, hopwise will
...
For more information about label setting in hopwise,
