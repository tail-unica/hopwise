...
- ``eval_args (dict)``:  This parameter have 4 keys: ``group_by``, ``order``, ``split``, and ``mode``,
  ...
  - ``split (dict)``: decides how we split the data in `.inter`. Now we support the following kinds of splitting strategies: ``['RS','LS','TS']``, which denotes the ratio-based data splitting, leave-one-out data splitting, and timestamp-based data splitting. If the key of ``split`` is ``RS``, you need to set the splitting ratio like ``[0.8,0.1,0.1]``, ``[7,2,1]`` or ``[8,0,2]``, which denotes the ratio of training set, validation set and testing set respectively. If the key of ``split`` is ``LS``, now we support three kinds of ``LS`` mode: ``['valid_and_test', 'valid_only', 'test_only']`` and you should choose one mode as the value of ``LS``. If the key of ``split`` is ``TS``, you need to set two timestamp cutoffs like ``[t1,t2]``, which denotes that the interactions with timestamp less than or equal to ``t1`` are used as training set, those with timestamp larger than ``t1`` and less than or equal to ``t2`` are used as validation set, and those with timestamp larger than ``t2`` are used as testing set. The default value of ``split`` is ``{'RS': [0.8,0.1,0.1]}``.
