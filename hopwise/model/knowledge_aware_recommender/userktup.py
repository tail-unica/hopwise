...
class UserKTUP(KnowledgeRecommender):
    ...
    def forward(self, user, item):
        ...
        user_entity_e = self.entity_embedding(user)
        item_entity_e = self.entity_embedding(self.n_users + item)
        user_e = user_e + user_entity_e
        item_e = item_e + item_entity_e
    ...
    def calculate_loss(self, interaction):
        ...
        item_align_loss = self.align_weight * alignLoss(
            self.item_embedding(item), self.entity_embedding(item + self.n_users), self.L1_flag
        )
        user_align_loss = self.align_weight * alignLoss(
            self.user_embedding(user), self.entity_embedding(user), self.L1_flag
        )
        align_loss = item_align_loss + user_align_loss
    ...
    def calculate_kg_loss(self, interaction):
        ...
        item_entity = entity[torch.logical_and(entity >= self.n_users, entity < self.n_users + self.n_items)]
        user_entity = entity[entity < self.n_users]
        item_align_loss = self.align_weight * alignLoss(
            self.item_embedding(item_entity - self.n_users), self.entity_embedding(item_entity), self.L1_flag
        )
        user_align_loss = self.align_weight * alignLoss(
            self.user_embedding(user_entity), self.entity_embedding(user_entity), self.L1_flag
        )
        align_loss = item_align_loss + user_align_loss
