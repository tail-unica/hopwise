...
class UserMKR(KnowledgeRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        # init embeddings
        self.user_embeddings_lookup = nn.Embedding(self.n_entities, self.embedding_size)
        ...
        for i in range(self.H):
            high_mlp_layers.append(self.embedding_size * 3)
        ...
        for i_cnt in range(self.L):
            self.user_cc_unit.add_module(f"user_cc_unit{i_cnt}", CrossCompressUnit(self.embedding_size))
        self.item_cc_unit = nn.Sequential()
        for i_cnt in range(self.L):
            self.item_cc_unit.add_module(f"item_cc_unit{i_cnt}", CrossCompressUnit(self.embedding_size))
        self.kge_mlp = MLPLayers(high_mlp_layers, self.dropout_prob, "sigmoid")
        self.kge_pred_mlp = MLPLayers([self.embedding_size * 3, self.embedding_size], self.dropout_prob, "sigmoid")
    ...
    def forward(
        ...
        self.item_embeddings, item_head_embeddings = self.item_cc_unit(
            [self.item_embeddings, head_embeddings]
        )  # calculate feature interactions between items and entities
        self.user_embeddings, user_head_embeddings = self.user_cc_unit(
            [self.user_embeddings, head_embeddings]
        )  # calculate feature interactions between items and entities

        self.head_embeddings = torch.cat([item_head_embeddings, user_head_embeddings], 1)
        ...
        if relation_indices is not None and task == "kge":
            # KGE
    ...
    def calculate_rs_loss(self, interaction):
        ...
        user_embeddings, item_embeddings, scores, scores_normalized = self.forward(
            ...
            item_indices=self.item_indices + self.n_users,
            head_indices=self.head_indices + self.n_users,
            relation_indices=None,
            tail_indices=None,
            task="rs",
        )
    ...
    def calculate_kg_loss(self, interaction):
        ...
        # inputs
        self.user_indices = interaction[self.HEAD_ENTITY_ID]
    ...
    def predict(self, interaction):
        ...
        outputs = self.forward(user, item + self.n_users, head + self.n_users, task="rs")
