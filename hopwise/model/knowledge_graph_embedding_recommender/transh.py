...
##################################################
Reference:
    Wang Z. et al. Knowledge Graph Embedding by Translating on Hyperplanes. In Twenty-Eighth AAAI Conference on Artificial Intelligence, June 2014.
"""
...
class TransH(KnowledgeRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.norm_vec = nn.Embedding(self.n_relations + 1, self.embedding_size)
    ...
    def forward(self, user, item):
        ...
        relation_ids = torch.tensor([self.n_relations]*user_e.shape[0],device=self.device)
        score = self._get_score(user_e, item_e, rec_r_e, relation_ids)
        return score
    ...
    def _get_score(self, h_e, t_e, r_e, rel):
        return -torch.norm(self.project(h_e, rel) + r_e - self.project(t_e, rel),p=2,dim=1)
    ...
    def full_sort_predict(self, interaction):
        ...
        relation_ids_user = torch.tensor([self.n_relations]*user_e.shape[0], device=self.device)
